...
agent.add_node("planner", get_events_node)
...
agent.add_edge("current_time", "classify_model")
agent.add_conditional_edges(
    "classify_model", route_classifier, {"planner": "planner", "reminder": "model"}
)

agent.add_conditional_edges(
    "model", should_continue, {"tool": "tool", "human_feedback": "human_feedback_reminder"}
)
agent.add_edge("human_feedback", "model")
agent.add_edge("tool", "refine_model")
agent.add_edge("refine_model", END)
agent.add_edge("planner", "scheduler")
agent.add_conditional_edges("scheduler",should_continue,{"tool": "tool", "human_feedback": "human_feedback_planner"}
)
...
app = agent.compile(interrupt_before=["human_feedback_reminder","human_feedback_planner"], checkpointer=memory)
...
initial_input = {
    "messages": [HumanMessage(content=input_)]
}
...
if isinstance(event["messages"][-1], AIMessage) and event.get("task_type") == "planner":
